...
本节以 `src/life_insurance_business_analysis_assistant/agent/graph.py` 的 `build_graph()` 及当前节点实现为准。普通模式包含 9 个业务节点；传入 `studio_context` 后使用 `ChatState`，增加 4 个聊天节点。下文源码路径均相对于 `src/life_insurance_business_analysis_assistant/`。
