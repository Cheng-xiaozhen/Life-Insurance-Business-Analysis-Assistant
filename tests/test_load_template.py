...
def test_load_template():
    ...
    for scenario_id, count in [("standard_premium_review", 5), ("value_review", 3)]:
        ...
        assert template["slot_definitions"]["机构范围"]["default"] == "全系统"
        assert "default" not in template["slot_definitions"]["年份"]
        assert template["steps"][0]["query_params"]["年份"] == "{{年份}}"
        assert state == before
        json.dumps(template, ensure_ascii=False)

    original = OmegaConf.to_container(OmegaConf.load(path), resolve=False)
    ...
    with TemporaryDirectory() as directory:
        ...
        shuffled["场景"][0]["分析思路"].reverse()
        shuffled["场景"][0]["输入参数"]["年份"]["默认值"] = 2026
        shuffled["场景"][0]["分析思路"][0].pop("分析模式")
        write(shuffled)
        ...
        for field, value, message in [
            ...
            invalid["场景"][0]["输入参数"]["月份"][field] = value
        ...
        for field, value, message in [
            ...
            invalid = deepcopy(original)
            invalid["场景"][0]["分析思路"][0][field] = value
            reject(invalid, message)
        reject({"场景": [original["场景"][0]] * 2}, "实际找到 2 个")
        reject({"场景": []}, "实际找到 0 个")
        reject({}, "'场景' 列表")
        write(original)
